...
def construct_optimal_solution(n: int) -> List[List[int]]:
    ...
    分块循环矩阵数学模型：
      令 k = n // 2。全盘矩阵划分为 4 个 k×k 分块：
      A = [[I_k + P_k, 0],
           [I_k,       I_k]]
      其中 I_k 为单位阵，P_k 为循环右移一位矩阵。
    坐标落子通式：
      1. 上半区 (0 <= r < k):
         c1 = r, c2 = (r + 1) % k
      2. 下半区 (k <= r < n, 设 i = r - k):
         c1 = i, c2 = k + i
    三重合法性保证（保证 100% 能放下且不冲突）：
      - 保证一（每格最多1枚）：因 k >= 2，模 k 下 (r+1)%k != r 恒成立；i != k+i 恒成立；上下半区行不交。所有格值为 0 或 1。
      - 保证二（每行严格为2枚）：上半区由 I_k+P_k 贡献 2 枚，下半区由两个 I_k 各贡献 1 枚，每行严格为 2 枚（偶数 > 0）。
      - 保证三（每列为奇数且能放下）：前 k 列得 2+1=3 枚，后 k 列得 0+1=1 枚，全为正奇数。列高度 n >= 4 > 3，最大列和 3 不超出列容量，必能容纳。
    """
    if n % 2 != 0 or n < 2:
