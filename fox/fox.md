...
- 参考 `reversetetris/` 的实现方式，使用 Web Audio API（振荡器合成）生成 BGM 循环与交互音效，无需额外音频文件。
- 通过页面上的 `#audio-btn`（🔇/🔊）切换静音状态，并使用 `localStorage` 记忆开关（`fox_audio_muted`）。
