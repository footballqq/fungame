...
- [x] 5.11 推子动作可视化与 AI 慢动作：新增 `playSlideAnimation` 滑动动画（被推棋子从来源方向平滑滑入、新子翻面入场），玩家与电脑推子均可见过程；AI 行动改为三阶段放慢（650ms 思考 → 800ms 取子高亮播报"第x行第y列，向x推入" → 500ms 推入动画）；引入 moveToken 令牌防重新开始后的滞后回调误结算；动画期间锁操作与取消
