...
### 1. 切换模型与教学引导 (Modes & Teaching)
系统顶部的导航栏允许您在四种经典组合模型间切换。系统现已集成**教学引导**模式，专为中小学生设计，通过递进式问答和枚举展示帮助学生理解原理。模型按照从易到难的顺序排列（默认进入 DD 模型）：
- **不同球 不同盒 (DD)**：对应指数分配 ($m^n$)。最符合直觉的日常分配模型。
- **相同球 不同盒 (ID)**：对应隔板法 (Stars and Bars)。
- **不同球 相同盒 (DI)**：对应第二类斯特林数 (Stirling Numbers II)。
- **相同球 相同盒 (II)**：对应整数分拆 (Integer Partitions)。最抽象的模型。

您可以通过界面上的“开启教学引导”/“进入自由探索”按钮，在教学问答和自由矩阵之间切换。
