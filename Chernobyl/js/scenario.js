// codex: 2026-06-07 增加背景说明、开篇切尔诺贝利事件介绍及核物理/核聚变科普选项
...
class ScenarioEngine {
    ...
    startStory() {
        ...
        this.triggerIntroNarration();
    }

    // 游戏开篇背景介绍
    triggerIntroNarration() {
        this.state.showDialogue(
            "历史日志：切尔诺贝利 (1986年4月25日)",
            "1986年4月25日，苏联乌克兰境内的切尔诺贝利核电站4号反应堆。此时，例行的发电厂年度停机维修即将展开。然而，在副总工程师迪亚特洛夫的违规要求下，该机组决定在停堆期间，测试发电机汽轮机在断电惯性转动下的余电，能否短时维持主水泵运转（大约40秒），直到应急备用柴油机发电。为了测试能顺利收集数据，迪亚特洛夫不惜下令切断应急堆芯冷却系统（ECCS），导致堆芯在数小时的电网调度延误中积聚了巨量的中子毒物——氙-135，反应堆正在陷入极度不稳定的‘反应堆毒化阱’。核能历史上最悲壮、最震撼的一幕正缓缓拉开帷幕...",
            [
                {
                    text: "【进入中子控制室】开始停堆降功试验准备",
                    callback: () => {
                        this.triggerStage1();
                    }
                }
    ...
    onPrizmaQueried() {
        this.state.showDialogue(
            "列昂尼德·托普图诺夫 (操作员)",
            "阿基莫同志！数据出来了……这太可怕了！计算显示我们的 ORM（操作反应性裕度）已经跌到了 12.0 根控制棒！安全红线是 15 根！核心已经被氙-135严重‘毒死’，对控制棒响应极其迟钝。此时我们必须立刻停堆，等待24小时直到氙衰退完毕！",
            [
                {
                    text: "📖 【核物理科普】什么是操作反应性裕度(ORM)与氙-135毒化？",
                    callback: () => {
                        this.showScienceXenon();
                    }
                },
                {
                    text: "阿基莫夫: 我同意。迪亚特洛夫同志，堆芯毒性太大，不能再试验了！",
                    callback: () => {
                        this.onAkimovRefuseXenon();
                    }
                }
            ]
        );
            ...
            [
                {
                    ...
                    callback: () => {
                        ...
                        this.state.showDialogue(
                            ...
                            [
                                {
                                    text: "📖 【核物理科普】什么是蒸汽正空泡反馈？",
                                    callback: () => {
                                        this.showScienceVoid();
                                    }
                                },
                                {
                                    text: "（疾呼）立刻拉开 AZ-5 安全罩！",
                                    callback: () => {
    ...
    onReactorExplode() {
        ...
        this.state.showDialogue(
            ...
            [
                {
                    text: "📖 【核物理科普】为什么紧急停堆按钮 AZ-5 反而引爆了堆芯？",
                    callback: () => {
                        this.showScienceScram();
                    }
                },
                {
                    text: "探查控制室废墟...",
    ...
    onSourceLocked() {
        this.state.showDialogue(
            ...
            [
                {
                    text: "📖 【核物理科普】核裂变污染与未来清洁核聚变能的区别",
                    callback: () => {
                        this.showScienceFusion();
                    }
                },
